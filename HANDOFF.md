...
### Step 24 — Maximum Open-Shift Cap (Stale Check-Ins No Longer Consumed as Next-Day Checkouts) ✓

**⚠ CORRECTION (added retroactively during Step 27):** as originally written, this step introduced a new, real bug that went undetected until 16 August production data was audited. Before this step, a worker could never have more than one open (checked-in, not-checked-out) record — any punch always closed whatever was open, so a single arbitrary "find one open record" database lookup was always safe. This step deliberately broke that invariant on purpose (a stale record is left open for manual review instead of being closed) — which means a worker CAN have several simultaneously-open records — but `findOpenRecord()` in `punch-resolution.ts` was never updated for that: it still fetched "one open record" with no ordering at all, so which one it returned was arbitrary once more than one existed. In production this surfaced three days after this step deployed as: real checkouts silently diverted into brand-new check-in rows instead of closing the worker's actual shift (mass "Missing checkout"), duplicate rows sharing the identical timestamp, and workers with 3+ shift rows in a single day. **A future reader must not repeat this mistake: any change that allows more than one row to satisfy an "is this open/current" query must also make the selection among them deterministic, not just correct in isolation.** Fixed in Step 27 below — read that section for the actual fix; the cap concept itself (documented just below) was correct and is preserved.

**The bug (live in production):** Step 19's rule was "a punch closes any open record, no matter how old" — correct for a genuine overnight shift closed the next morning, but wrong when a worker simply forgot to punch out: their *next day's* check-in was silently consumed as *yesterday's* checkout, producing a fake ~24-hour shift on day 1 and leaving the worker with no record at all — and Rs 0 pay — for day 2, which they actually worked.

**The fix:** new setting `maxOpenShiftHours` (default `14`, no schema migration needed — reuses the existing `app_settings` key/value table, same pattern as `payrollCutoffTime`). `getMaxOpenShiftHours()` added to `src/lib/settings.ts`. In `resolvePunch()` (`src/lib/punch-resolution.ts`), when an open record is found:
- gap between the punch and the open record's `checkInAt` ≤ cap → **unchanged behavior**, closes it as a checkout.
- gap > cap → the stale record is **left open, untouched** (keeps surfacing as "⚠ Missing checkout" on the Attendance page for manual correction) and the punch instead becomes a **new check-in**, via the exact same shiftSequence logic already used for same-day doubles (Step 19).
- Both `/api/kiosk/attend` and `/api/biometric/punch` inherit this automatically since both call `resolvePunch()` — no logic duplicated in either route. Biometric duplicate/idempotency detection (exact-timestamp match on the open record's `checkInAt` or the latest record's `checkOutAt`) is untouched and still runs first, before the staleness check.

**Decision — `audit_log.actor_user_id` made nullable:** the spec calls for an audit row on every stale-skip, with `actorUserId: null` since it's a system decision, not a user action — but that column was `NOT NULL` with a foreign key to `users`. I considered inserting a fake "system" user row instead, but rejected it: it would need to satisfy every `users` table constraint (unique email, password hash, a role) and creates a standing fake account that could be confused for a real login or picked up by other user-listing code. Instead I dropped the `NOT NULL` constraint — migration `drizzle/0005_futuristic_switch.sql`, one line: `ALTER TABLE "audit_log" ALTER COLUMN "actor_user_id" DROP NOT NULL;`. Every existing human-triggered audit write elsewhere in the app (`correctAttendance`, `markAbsent`, `markLeave`, user/payroll actions) still always passes a real `actorUserId` — nothing about their behavior changes. The new `stale_shift_skipped` row records `entityType: "attendance_record"`, `entityId` = the **stale** record's id, `beforeJson: { checkInAt }`, `afterJson: { newCheckInAt }` (both ISO strings, matching how other audit writers serialize dates), `actorUserId: null`.

**Settings page (`/dashboard/settings`):** second card, "Maximum Open Shift" — number input, 1–24, validated server-side in a new `updateMaxOpenShiftHours()` action (same admin-only gating, same save-button/toast pattern as the existing payroll cutoff field). Helper text explains the behavior in plain language. Both cards' inputs got `w-full sm:w-40` (was a fixed `w-40`) so they don't force horizontal scroll on narrow phones — matching the Step 23 mobile pattern; this page is a simple stacked card form so no table→card treatment was needed.

**Pure logic extracted for testability:** `isOpenRecordStale(openCheckInAt, punchTimestamp, maxOpenShiftHours)` lives in a new dependency-free `src/lib/open-shift-cap.ts`, not inside `punch-resolution.ts` — that file imports the database client at module load time (via `shift-resolution.ts` → `db.ts`), so it can't be imported standalone without a live `DATABASE_URL`, which would break "pure-function tests, no database connection." `punch-resolution.ts` imports the same function from `open-shift-cap.ts`, so there's exactly one implementation, just relocated.

**Tests:** `src/lib/punch-resolution.test.ts` (new, run via `npx tsx src/lib/punch-resolution.test.ts`) — 7 tests, all pure, no DB: 8h-after-check-in closes; 20h-after-check-in becomes a new check-in and leaves the old one open; exactly-at-the-14h-boundary still closes (inclusive); one-minute-past-the-boundary is stale; a 13h overnight checkout still closes correctly; plus two cases with a custom (non-default) cap. **Full suite: 59/59 passing** (52 pre-existing in `attendance.test.ts`, unchanged, + 7 new) — confirmed nothing else broke.

**⚠️ Manual step required before this is live — please do this before/at your next deploy:**
...
### Step 26 — Configurable Kiosk Refresh Interval (Reduce Neon Compute Usage) ✓

**Why:** Neon free tier was at 80% of its 100 compute-hour monthly allowance, and the usage graph showed the database staying continuously awake through the whole operating day instead of waking in short bursts. The kiosk tablet is left running all day and was polling `/api/kiosk/workers` every 5 minutes (hardcoded, added in Step 13), which kept resetting Neon's auto-suspend timer. The worker list barely changes day to day, so 5-minute polling was far more aggressive than needed.

**The fix:** new setting `kioskRefreshMinutes` (default `60`, range 5–240, no schema migration — same `app_settings` key/value pattern as `payrollCutoffTime`/`maxOpenShiftHours`). `getKioskRefreshMinutes()` added to `src/lib/settings.ts`, clamping/falling back to 60 exactly like `getMaxOpenShiftHours()` falls back to 14.

- `src/app/kiosk/page.tsx` reads the setting **once, server-side**, at page load, and passes it to `KioskClient` as a new `refreshMinutes` prop — deliberately not a second client-side poll (that would have reintroduced the exact problem being fixed).
- `src/app/kiosk/kiosk-client.tsx`'s `setInterval` for `refreshWorkers` now uses `refreshMinutes * 60 * 1000` instead of the hardcoded `5 * 60 * 1000`; `refreshMinutes` added to that `useEffect`'s dependency array.
- Nothing else in the offline path touched: `kiosk-idb.ts`'s `worker_cache`/`punch_queue` stores, the date-based midnight cache invalidation, the offline punch queue, and the reconnect `syncQueue()` auto-sync all still run exactly as before — the only thing that changed is how often the online-refresh timer fires.

**Settings page (`/dashboard/settings`):** third card, "Kiosk Worker List Refresh" — number input labeled "Kiosk worker list refresh (minutes)", 5–240, validated server-side in a new `updateKioskRefreshMinutes()` action (same admin-only gating, same save-button/toast pattern, same `w-full sm:w-40` mobile-safe input width as the other two cards). Helper text: "How often the kiosk re-fetches its worker list from the server. A longer interval reduces database usage. Newly added workers or PIN changes may take up to this long to appear on the kiosk."

**Audit — what else polls or hits the database on a timer (reported, not changed, per instructions):**
- Searched the whole `src/` tree for `setInterval`, `setTimeout`, `useSWR`/`react-query`, `router.refresh()`, and Next.js `revalidate`/`next: { revalidate }` fetch options. Before this step there were exactly two `setInterval` calls in the entire app, both in `kiosk-client.tsx`: the on-screen clock (1-second tick, pure client state, no network/DB — harmless) and the worker-list refresh just fixed above. No other page has a client-side polling loop, and no route uses time-based ISR revalidation.
- **The actual bigger contributor is likely outside this app's own polling and is not something this step touched:** `zkteco-bridge/index.js`, the biometric terminal-PC bridge (Step 22), runs via Windows Task Scheduler **every 5 minutes, 24/7 year-round** — not just during business hours like the kiosk tablet. Every single run unconditionally POSTs to `/api/biometric/heartbeat`, which does one `app_settings` read + one write via `recordBiometricHeartbeat()` — that's ~288 database round trips per day, continuously, including nights and days with no shift running. By contrast, `/api/biometric/punch` is only called when the bridge finds new fingerprint punches on the device (an empty result skips the fetch entirely), so it doesn't add continuous load the way the heartbeat does.
- Recommendation for you to decide on (not implemented here): if Neon usage is still high after this step's change takes effect, the heartbeat's 5-minute/24-hour cadence is the next place to look — either widening the Task Scheduler interval, or only sending heartbeats during configured operating hours. I did not change `zkteco-bridge` in this step since it runs unattended on a terminal PC and any change there needs a manual redeploy to that machine, and you asked to decide before I touch anything else.
- No Vercel cron config (`vercel.json`) exists, and no page uses `export const revalidate`, so there are no other timer-driven sources of DB load beyond the two called out above.

**Verified:**
- `npx tsc --noEmit`: zero errors across the whole project.
- Full test suite: **94/94 passing** (52 attendance + 7 punch-resolution + 35 access-control, all pre-existing and unchanged) — this step touched no tested logic, so no new tests were needed; confirmed nothing broke.
- `npm run build`: 24 routes, same count as before this step, compiles cleanly.
- No database migration needed — `app_settings` already supports arbitrary keys, exactly like Step 24's `maxOpenShiftHours`.
- Offline behavior explicitly re-read and confirmed unchanged: `kiosk-idb.ts` (cache + punch queue), the midnight date-invalidation check in `getCachedWorkers`, `enqueuePunch`/`syncQueue`'s reconnect drain, and the offline indicator dot were not modified — only the refresh interval's source (hardcoded constant → app_settings-backed prop) changed.
- **What I could not verify by clicking through the live app** (no browser tool, no kiosk hardware in this session): actually loading `/kiosk?terminal=...` and watching the refresh fire on the configured cadence, and confirming the new Settings card saves and re-renders correctly in a real logged-in admin session. Recommend the manual check below.

---

### Step 27 — Deterministic Open-Record Resolution, Auto-Close Stale Shifts ✓

**Fixes the regression Step 24 introduced** (see the correction note added to the Step 24 section above). Production on 16 August showed 69 of 71 workers with "Missing checkout", duplicate rows sharing identical timestamps, and workers with 3+ shift rows in a day. Root cause: once a worker could have more than one simultaneously-open attendance record, `findOpenRecord()`'s unordered "give me one" lookup could return the wrong one — an old abandoned record instead of the worker's actual current shift — silently diverting real checkouts into brand-new check-in rows.

**The fix — two new pure, dependency-free functions in `src/lib/open-shift-cap.ts`** (alongside the existing `isOpenRecordStale` from Step 24), so the actual decision logic is unit-testable without a database connection:
- `resolveOpenRecords(openRecords, punchTimestamp, maxOpenShiftHours)` — given ALL of a worker's currently-open records, sorts them by `checkInAt` and returns `{ toClose, current }`: every record more than `maxOpenShiftHours` old is auto-closed (`toClose`); of what's left, the single most recent is `current` — the record this punch should close as a checkout, or `null` if none survive (meaning this punch is a new check-in). This replaces the old single-record `.limit(1)` lookup with a full sweep, so it also cleans up any backlog of multiple abandoned records for a worker in one pass, not one per future punch.
- `findDuplicateRecord(records, punchTimestamp)` — checks a punch against an exact `checkInAt` or `checkOutAt` match across ALL of a worker's records (open, closed, or auto-closed), independent of which record any "open" lookup would return. This is what makes duplicate detection correct now that Step 20's chunked bridge upload is confirmed (see below) to legitimately resend already-applied punches after a partial batch failure — resend-safety no longer depends on luck.

**`punch-resolution.ts` rewritten around these two functions:** `resolvePunch()` now (1) checks for a duplicate first, against the worker's full record set, before looking at open-record state at all; (2) fetches every open record for the worker (`findOpenRecords`, renamed from the old singular `findOpenRecord`, now filtered to `status = "present"` and unordered by design since the pure function does the sorting); (3) auto-closes every stale one and, if a genuine current shift survives, closes it as the checkout; otherwise falls through to the existing new-check-in path, unchanged.

**Representing "closed but no real checkout" — the design decision:** added `"incomplete"` as a new allowed value of the existing `status` text column (`present | absent | leave | incomplete`). No migration — confirmed via every prior migration file that `status` has never had a database-level CHECK constraint, only a TypeScript union (same situation as the `role` column in Step 25). An auto-closed record gets `status: "incomplete"`, `checkoutMissing: true`, `checkOutAt` left `null` forever (never invented), `checkInAt` and every other field untouched. This was the cleanest option given the existing schema:
- It's automatically excluded from `findOpenRecords()`'s `status = "present"` filter, so it can never be picked up again by a future punch — permanently out of the running, which was the core requirement.
- The Attendance page's live "Missing checkout" derivation (`checkInAt` set, `checkOutAt` null, shift end has passed) already picks it up with zero code change, so it keeps surfacing for manual correction exactly like an ordinary missing checkout does.
- Reports' existing `status === "present"` filters for "Present days" and "Total Shifts" already exclude it with zero code change — confirmed by reading both `reports/page.tsx` and `reports/[workerId]/page.tsx` before writing this, not assumed.
- A supervisor corrects it exactly the way "Missing checkout" rows are already corrected today — open the row on the Attendance page, enter the real check-in/check-out times, save; `correctAttendance()` resets `status` back to `"present"` with no special-casing needed, since it already operates on a specific record by id.

**Compliance visibility (the actual point of this system right now) — audited and fixed everywhere "incomplete" can reach the screen:**
- `StatusBadge` in both `attendance-client.tsx` and `reports/[workerId]/detail-client.tsx` had an unconditional final `else` that assumed "not present, not absent" meant "leave" — so before this fix, an auto-closed compliance failure would have silently rendered as a blue "Leave" badge, i.e. looked like an *approved absence*, which is the opposite of what it is. Both now have an explicit `"incomplete"` branch rendering a distinct orange "Did not check out" badge.
- Attendance page summary bar: new always-visible "`N` did not check out" tally (orange), counted per distinct worker per day exactly like present/absent/leave, so it can't hide inside "no record". New "Did not check out" option added to the status filter dropdown.
- Reports list page and CSV export: new "Did Not Check Out" column alongside Present/Absent/Leave/No Record, in the desktop table, mobile cards, and the downloadable CSV (headers, data rows, and the totals row) — all three render paths in `reports-client.tsx` were updated together so none of them could drift out of sync with the others.
- Reports worker-detail page: new orange "Did not check out" summary chip alongside Present/Absent/Leave.
- Audit Log: new `stale_shift_auto_closed` action (replaces Step 24's `stale_shift_skipped`, which is no longer written) with its own label and orange color, instead of falling back to the raw snake_case string.

**How the counts were decided (matches what was asked for directly):** "incomplete" is excluded from every "Present" tally and from Reports' "Total Shifts" — it never got a real checkout, so it must not read as a completed shift. It is also excluded from "Absent" (the worker did show up) and from "No record" (the day isn't empty, there's a real check-in). Instead it gets its own dedicated, always-visible count everywhere Present/Absent/Leave are shown, so a compliance failure has a permanent home in the summary rather than being folded into an existing bucket or disappearing from the totals.

**`payroll-report.ts`:** `shiftsWorked` now excludes `status === "incomplete"` records — confirmed by reading the function before changing it that `recs.length` previously counted every row regardless of checkout state, so this is a narrow, explicit exclusion, not a broader redesign. The existing `checkoutMissing` scan is left inclusive of incomplete records on purpose, so payroll still warns about the compliance gap before a closing is finalized, even though that shift won't be paid. Payroll is not live yet, per the prompt's own note, so this was verified by reading, not by running a real closing.

**Full re-audit of "does this assume at most one open record" (as requested) — every site found and its disposition:**
| Site | Assumed ≤1 open record? | Fixed |
|---|---|---|
| `punch-resolution.ts` duplicate check | Yes — compared only against whichever record `findOpenRecord()` returned | Yes — `findDuplicateRecord()` now checks the worker's full record set |
| `punch-resolution.ts` staleness cap | Yes — evaluated only the one arbitrary record | Yes — `resolveOpenRecords()` sweeps all open records |
| Kiosk "checked in" tile — `kiosk/page.tsx` | No open-record-*count* assumption, but didn't exclude auto-closed records | Yes — added `status = "present"` to the open-shift query |
| Kiosk "checked in" tile — `api/kiosk/workers/route.ts` | Same as above | Yes — same fix |
| Attendance page (`page.tsx`, `attendance-client.tsx`) | No — already handles multiple same-day records per worker (Step 19); "Missing checkout" is derived live per-record, not from a singular lookup | No code change needed to the query; badge/summary fixes above were about *display*, not the open-record assumption |
| Reports (list + detail) | No — status-filtered aggregates, never looked at "open" state directly | No change needed beyond the incomplete-status counting decisions above |
| `payroll-report.ts` | No open-record lookup at all — operates on a closing-window date range | `shiftsWorked` exclusion above is a counting fix, not an open-record fix |
| Dashboard `page.tsx` (today summary) | No — same live-derivation pattern as the Attendance page | No change needed |

**Historical backlog — confirmed self-healing, not separately repaired (out of scope for this step, as instructed):** the bridge and kiosk are still running. `resolveOpenRecords()` sweeps and closes *every* stale open record for a worker on their very next punch, not just the most recent one — so the already-corrupted 13–16 August backlog cleans itself up automatically as each affected worker punches again, without a separate backfill script. A worker who doesn't punch again for a while will keep showing as "Missing checkout" (which is factually true) until they do. No historical rows were modified directly, and Bug #1 (attendance.ts timezone mislabeling) and the biometric bridge were not touched, per the prompt's explicit scope.

**Tests:** `src/lib/punch-resolution.test.ts` extended with 10 new pure-function assertions (no DB) covering exactly what was asked: two open records → punch closes the most recent, not the older one, and the result doesn't depend on database row order; a stale open record → auto-closed, and the worker's next punch (simulated with an empty open-record set) is not misrouted; a resent punch → recognised as a duplicate even with multiple open records present, including one already auto-closed to "incomplete"; and the ordinary single-open-record and overnight-shift-checkout paths → unchanged. No existing test fixtures encoded the wrong assumption (the Step 24 tests only ever exercised `isOpenRecordStale` in isolation, which is unaffected by this step), so nothing needed correcting, only extending. **Full suite: 104/104 passing** (52 attendance + 17 punch-resolution [7 pre-existing + 10 new] + 35 access-control) — confirmed nothing that passed before now fails.

**Verified:**
- `npx tsc --noEmit`: zero errors across the whole project.
- `npm run build`: 24 routes, same count as before this step, compiles cleanly.
- **No database migration needed** — `status` has never had a database-level CHECK constraint in any prior migration (confirmed by reading every migration file, not assumed), so adding `"incomplete"` as an allowed value is a TypeScript-level change only, identical in kind to how Step 25 added the `"viewer"` role.
- **What I could not verify by clicking through the live app** (no browser tool, no kiosk/bridge hardware in this session): actually watching a real duplicate biometric resend or a real multi-open-record worker get resolved correctly against the live production database. The fix is exercised via the pure-function tests above and confirmed compiling/building cleanly against the real schema; recommend watching the Attendance page's new "did not check out" tally and the Audit Log's new `stale_shift_auto_closed` entries over the next few sync cycles to confirm the backlog is draining as described.

---

### Step 28 — Conditional Heartbeat Writes to Reduce Database Compute ✓

Neon hit 100% of its free-tier compute allowance. Step 26 had already called out the biometric bridge's heartbeat as the likely biggest remaining contributor: it runs on Windows Task Scheduler around the clock (previously every 5 minutes, being widened to 30 minutes separately by the user on the terminal PC — not touched in this step) and, before this change, `recordBiometricHeartbeat()` in `src/lib/settings.ts` wrote to `app_settings` on *every single call*, even when the run reported nothing new (the common case — most runs find no new punches and report the same static "No new punches since last sync." message every time).

**The fix — `recordBiometricHeartbeat()` now skips the write when the incoming heartbeat is identical in substance to what's already stored.** "Substance" is `success`, `message`, and `recordsSynced` — comparing these covers both a down→up (or up→down) transition (`success` flips) and any real new activity (`message`/`recordsSynced` change together whenever the bridge actually syncs punches or hits a new error, since the bridge's own message string embeds the count). A run that just repeats "still alive, nothing new" no longer touches the database at all — it validates the secret, reads the one existing `app_settings` row, sees nothing changed, and returns `{ ok: true }` with no write.

**Preserving "last seen" freshness without writing on every ping:** a pure "only write on change" rule would let `ranAt`/`lastSuccessAt` go stale indefinitely during a long quiet spell (e.g. overnight with zero punches, which is normal, not a problem) — and the Dashboard's health card needs that timestamp to detect an actually-dead bridge. Chose the lightest fix: `recordBiometricHeartbeat()` also force-writes a refresh at least once per `HEARTBEAT_KEEPALIVE_MS` (60 minutes) even when nothing changed, so "last seen" can lag reality by at most about an hour instead of drifting forever. **Tradeoff, explicitly accepted:** the Dashboard's "last successful sync was X ago" is now accurate to within roughly an hour rather than to the minute. To match, `src/app/dashboard/page.tsx`'s "Bridge may be down" threshold was widened from 15 to 90 minutes (`DOWN_THRESHOLD_MS`) — comfortably above the ~60-minute keepalive lag plus one bridge run interval, so a healthy-but-quiet bridge no longer trips a false warning, while an actually-dead bridge (no heartbeats arriving at all, changed or not) is still caught, just somewhat later than before. Net effect: heartbeat writes drop from one every 5–30 minutes around the clock to, in the common case, at most one per hour — with a full detection story preserved.

Auth (`x-bridge-secret` header check) and the request-validation logic in `src/app/api/biometric/heartbeat/route.ts` are unchanged; only what happens *after* validation (whether to write) changed, entirely inside `settings.ts`.

**No database migration** — this reuses the existing `app_settings` key/value row (`biometricSyncStatus`) exactly as before; no schema or column changes.

**Verified:**
- Full test suite: **104/104 passing** (52 attendance + 17 punch-resolution + 35 access-control, run via `npx tsx <file>` per this project's convention since `npm test` isn't wired up) — this step touched no tested pure-function logic (the change is in a DB-backed function with no existing unit tests, consistent with how the rest of `settings.ts` is untested), so nothing broke.
- `npx tsc --noEmit`: zero errors.
- `npm run build`: 24 routes, same count as before this step, compiles cleanly.
- **What I could not verify by clicking through the live app** (no browser tool, no bridge hardware in this session): watching a real bridge send a "no change" heartbeat and confirming no row is written, and watching the Dashboard card render correctly with the new 90-minute threshold. Recommend the manual check below.

**Manual check on your phone:** open `/dashboard` as an admin after the bridge's next few runs. The "Biometric Sync" card and "last successful sync was X ago" text should look the same as before — the only difference is it may now show up to roughly an hour "behind" during quiet periods instead of updating every few minutes, and it won't warn "Bridge may be down" unless it's actually been quiet for over 90 minutes.

---

---

### Step 29 — Timezone Mislabeling Fix: computeLate/computeAllFlags (Bug #1) ✓

**Root cause found and fixed.** `attendance.ts`'s internal `buildUtcDate(workDate, timeStr)` built shift start/end instants by taking the shift's stored `"HH:MM"` (a Pakistan **local wall-clock** time — confirmed via the shift editor's plain `<input type="time">` with no conversion, and via `HANDOFF.md`'s own description of shifts as being in Pakistan local time) and appending it directly to a `...Z` (UTC) ISO string — i.e. it silently treated "09:00" as 09:00 UTC instead of 09:00 PKT. `checkInAt`/`checkOutAt` are genuine UTC instants (kiosk uses `new Date()`, the bridge parses the punch's own ISO timestamp) — those were never wrong, ruling out the server clock as the culprit, as already suspected. `payroll-report.ts`'s `computeClosingWindow()` already does this conversion correctly via its own `pktToUtc()` (hardcoded UTC+5, no DST); `attendance.ts` was simply missing the equivalent conversion entirely — not double-applied, not reversed, just never applied. The result is a **constant, fixed +300 minute (5-hour) misregistration** of every shift boundary, identical for day shifts and midnight-crossing night shifts alike (unrelated to which calendar day a shift falls on), which:
- suppressed lateness almost everywhere (the "late" threshold sat ~5h later than the real shift start, so a genuinely late check-in almost always landed before it) — `isLate` only ever flipped **false → true** in the backfill dry run below, never the other direction,
- suppressed overtime almost everywhere (same reason, at the other end of the shift),
- and fabricated early-leave flags/minutes for on-time or even slightly-late checkouts (the "early" threshold sat ~5h later than the real shift end) — `leftEarly` only ever flipped **true → false**, never the other direction.

**Fix:** `buildUtcDate()` now converts the same way `payroll-report.ts`'s `pktToUtc()` does — subtract a `PKT_OFFSET_MINUTES` (300) constant before building the UTC instant. Both files now apply the identical, single, correctly-directed conversion; worth a follow-up refactor to share one `pktToUtc` helper between them so they can't drift apart again, not done here since it wasn't asked for.

**Test fixtures fixed too, not just left passing:** `attendance.test.ts`'s `d()` helper previously parsed its ISO-like fixture strings as literal UTC ("Zulu"), which is exactly the bug — the tests passed only because they were self-consistent with the wrong behavior, never because they verified a real PKT-instant against a real local shift time. `d()` now independently converts each fixture string (written as a PKT wall-clock reading, e.g. `"2024-06-04T06:05:00"` = 6:05 AM PKT) to its real UTC instant via its own formula, not by calling into `buildUtcDate()` — so the test suite now actually exercises the PKT→UTC conversion end to end, and would have caught this bug (or would catch a regression of it — wrong sign, wrong offset, forgetting it again) instead of silently agreeing with it. No individual assertions needed to change: the fixture values are unchanged, only how they're interpreted.

**Historical backfill — dry run only, not applied.** Every existing `attendance_records` row was computed under the bug, so `isLate` / `lateMinutes` / `leftEarly` / `earlyLeaveMinutes` / `overtimeMinutes` are wrong on essentially every row with a check-in or check-out (`workedMinutes` and `checkoutMissing` are unaffected — the former is a raw instant-to-instant diff, the latter is re-evaluated live elsewhere and wasn't touched). Added `scripts/backfill-attendance-flags.ts` — standalone, **not** part of the Next.js app (no route, no server action), run manually via `npx tsx scripts/backfill-attendance-flags.ts`. Defaults to dry-run (SELECT + diff + report only); requires an explicit `--apply` flag to write, and even then re-fetches and re-checks each row immediately before writing so a row a supervisor corrects between the dry run and the real run isn't clobbered with a stale diff. `--csv <path>` writes the full per-row diff for review instead of only the console's first-25 sample.

**Dry run against the live database (read-only, nothing written), run this step:**
- 4,039 total `attendance_records` rows scanned; 180 skipped (no resolved shift to compare against — absent/leave records with nothing to check); 1,084 already correct (no check-in/check-out, or no crossing changed); **2,775 rows would change.**
- Changes by field: `isLate` 894 · `lateMinutes` 2,038 · `leftEarly` 122 · `earlyLeaveMinutes` 612 · `overtimeMinutes` 1,089.
- Sanity-checked the diff distribution against the "constant 300-minute offset" theory (not just eyeballed): for `overtimeMinutes`, 971 of the changed rows shift by exactly **+300**; for `earlyLeaveMinutes`, 490 shift by exactly **-300**; for `lateMinutes`, 1,149 shift by exactly **+300** — the remaining, non-±300 deltas are exactly the boundary-crossing cases where the old or new value is clamped at 0 (e.g. a truly-20-minutes-late check-in that the bug reported as `lateMinutes: 0` now reports `20`, a delta smaller than 300). `isLate` only ever flips `false → true` (894 times, 0 in the other direction) and `leftEarly` only ever flips `true → false` (122 times, 0 in the other direction) — both exactly match the predicted one-directional bias above. This is strong confirmation the fix and the backfill logic are both correct, not just plausible.
- Full per-row CSV of the dry run is saved to the session scratchpad, not committed to the repo (it's a point-in-time report, not project source).

**Backfill applied — 2026-09-17.** Approved after review of the dry run above. Before writing, took a Neon branch snapshot for rollback safety: `pre-attendance-backfill-2026-09-17` (branch id `br-floral-darkness-aoj9ldtf`), branched off `production` (`br-lively-brook-aol2524s`) at the pre-backfill LSN, via `neonctl branches create`. Not deleted — left in place as a restore point; safe to remove once the corrected figures have been spot-checked in the live app.

Ran `npx tsx scripts/backfill-attendance-flags.ts --apply`. The first invocation hit a transient network error (`ECONNRESET` / "fetch failed" from the Neon HTTP driver) partway through the per-row update loop, after applying roughly 689 of the then-2,775 changed rows — the wrapping shell pipeline still reported exit code 0 (masked by a trailing `tail`/`tee`), so this was caught by re-running the dry run afterward and comparing counts, not by trusting the exit code. The script's design made this a non-issue: every invocation re-SELECTs and re-diffs current database state from scratch before writing, so it's naturally idempotent and resumable — rows already corrected simply stop appearing in the `changed` set. Re-ran `--apply` a second time for the remainder (2,098 rows, including a handful of new check-ins that had landed on the still-old-buggy production deploy in the meantime — expected, since this is a live app and the code fix hadn't been pushed/redeployed yet); it completed cleanly this time: **"Applied 2098 row updates. Skipped 0 rows that changed since the dry run."**

**Final verification (fresh dry run after both apply runs):** 4,051 total rows now on file (12 more than the original 4,039 — new live check-ins during the session, handled correctly); 180 skipped (unchanged from the original — same absent/leave rows with no shift to compare against); **3,871 unchanged (already correct) · 0 changed remaining.** Total rows actually corrected across both apply runs: 2,787 (the original 2,775 plus 12 new rows created mid-session that also needed the same fix). No errors on the final run; no rows skipped as stale/raced.

**Verified:**
- Full test suite: **104/104 passing** (52 attendance + 17 punch-resolution + 35 access-control) — the 52 attendance tests now genuinely exercise the corrected PKT conversion (see above), not merely re-pass unchanged.
- `npx tsc --noEmit`: zero errors across the whole project, including the new backfill script (it's inside `tsconfig.json`'s existing `**/*.ts` include).
- `npm run build`: 24 routes, same count as before this step, compiles cleanly — this step touched no route.
- Post-backfill dry run confirms **0 rows remaining changed** — the database now matches what the corrected pure functions compute from each row's actual `checkInAt`/`checkOutAt`/shift data.
- **What I could not verify by clicking through the live app** (no browser tool in this session): watching the Attendance/Reports pages actually render the corrected lateness/overtime numbers for a real worker. Recommend spot-checking a few workers' history on `/dashboard/attendance` and `/dashboard/reports` against the CSV diffs in the scratchpad before deleting the Neon rollback branch.

`zkteco-bridge`, the biometric bridge, and every other step's scope were left untouched, per instructions. **The code fix (`attendance.ts`) still needs to be deployed** (pushed to `main`, then Vercel's normal deploy) for new check-ins going forward to stop needing this same correction.

---

## All 29 steps complete. App is in production; the attendance/payroll pipeline correctly handles double shifts end-to-end, a latent stale-data caching bug was found and fixed, the biometric bridge no longer fails wholesale on large first-run backfills, the Attendance page supports instant client-side search/status/department filtering, admins can see biometric bridge health directly on the Dashboard, the 5 most-used pages (plus the dashboard shell/sidebar) are usable in portrait on a phone without rotating to landscape, a forgotten checkout no longer silently swallows the following day's check-in, a third terminal-scoped read-only "viewer" role now exists alongside admin and supervisor, the kiosk's worker-list refresh interval is now admin-configurable (default 60 min, was hardcoded at 5) to reduce Neon compute usage, open-record resolution is now deterministic — a worker can no longer have an abandoned shift silently hijack a later punch, and a stale shift auto-closes as a clearly visible "did not check out" compliance record instead of vanishing into the wrong row, the biometric bridge's heartbeat now only writes to the database when something actually changed, cutting a write-every-few-minutes-around-the-clock pattern down to roughly hourly at most — and the attendance engine's timezone mislabeling bug (shift start/end times treated as UTC instead of Pakistan local) is fixed going forward, with the historical backfill reviewed, approved, and applied — 2,787 `attendance_records` rows corrected, verified against a fresh 0-changed dry run, with a Neon branch snapshot kept as a rollback point.
